...
Usage:
    Identifi_cluster.py path/to/O_blastout.m6   path/to/gff_file path/to/faa_file
    ...
    for term_num in  range(len(m6_file_filted)):
        if term_num == 0:
            ...
            if break_judgment(gff_file_list, len_per_gene, gene_name1, gene_name2):
                ...
                print "Predicted gene cluster", str(cluster_num) ,": " ,cluster_start, "\tto\t" , cluster_end,"\t(", seq_id,":" ,cluster_start_num, "--",cluster_end_num,")\tGene number of cluster: ",int(len(cluster_faa_list))
